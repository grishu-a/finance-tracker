...
export function LedgerTrendChart({ data }: { data: TrendPoint[] }) {
  return (
    <ResponsiveContainer width="100%" height={260}>
      <BarChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: 0 }} barGap={4}>
        ...
        <YAxis
          ...
          axisLine={false}
          tick={{ fill: "var(--muted-foreground)", fontSize: 12 }}
          tickFormatter={(v: number) => formatNpr(v)}
          width={64}
        />
        <Tooltip content={<ChartTooltip />} cursor={{ fill: "var(--muted)" }} />
        <Legend verticalAlign="top" align="right" height={32} wrapperStyle={{ fontSize: 12, color: "var(--muted-foreground)" }} />
